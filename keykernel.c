...
{
	unsigned int line_size = 2 * 80; // Elements * Columns
...
void clear_screen(void)
{
	...
	while (i < (lines*columns*elements)) {
		vidptr[i++] = ' ';
		vidptr[i++] = 0x02;
	}
}
...
void keyboard_handler_main(void)
{
	...
	if (status & 0x01) {
		...
		vidptr[current_loc++] = kbdus[(unsigned char) keycode];
		vidptr[current_loc++] = 0x02;
	}
}
...
void kmain(void)
{
	const char *str = "This is a simple kernel that supports a keyboard:";
	clear_screen();
	kprint(str);
	kprint_newline();
	kprint_newline();
